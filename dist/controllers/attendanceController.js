...
exports.markAttendance = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const markedBy = new mongoose_1.Types.ObjectId(req.user.userId); // Convert string to ObjectId
...
exports.getTodayProjectAttendance = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const project = await projectModel_1.Project.findById(projectId)
        .populate("assignedWorkers", "_id firstName lastName profileImage phoneNumbers" // Added _id to the field selection
    )
...
exports.getAttendanceSummary = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    users.forEach((user) => {
        totals[user._id.toString()] = attendanceRecords.filter((record) => record.user._id.toString() === user._id.toString() && record.present).length;
    });
    res.status(200).json(new apiHandlerHelpers_1.ApiResponse(200, {
        type,
        dates: uniqueDates,
...
exports.getNormalMonthlyAttendance = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    res
        .status(200)
        .json(new apiHandlerHelpers_1.ApiResponse(200, attendance, "Monthly normal attendance retrieved successfully"));
});
